...
import cn.miaow.framework.util.file.FileUploadUtils;
import cn.miaow.framework.util.file.MimeTypeUtils;
...
public class SysProfileController extends BaseController {
    ...
    public AjaxResult updateProfile(@RequestBody SysUser user) {
        ...
        if (StringUtils.isNotEmpty(user.getPhoneNumber()) && userService.checkPhoneNotUnique(user)) {
            return error("修改用户'" + user.getUserName() + "'失败，手机号码已存在" );
        }
        if (StringUtils.isNotEmpty(user.getEmail()) && userService.checkEmailNotUnique(user)) {
    ...
    public AjaxResult avatar(@RequestParam("avatarFile" ) MultipartFile file) throws Exception {
        if (!file.isEmpty()) {
            LoginUser loginUser = getLoginUser();
            String avatar = FileUploadUtils.upload(MiaowConfig.getAvatarPath(), file, MimeTypeUtils.IMAGE_EXTENSION);
